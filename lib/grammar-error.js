...
class GrammarError extends Error {
  ...
  format(sources) {
    ...
    function offsetStart(location) {
      return (typeof location.source.offset === "function")
        ? location.source.offset(location.start) // GrammarLocation
        : location.start;
    }
    ...
    function entry(location, indent, message = "") {
      ...
      const offset_s = offsetStart(location);
      if (src) {
    ...
    function formatProblem(severity, message, location, diagnostics = []) {
      ...
      if (location) {
        maxLine = diagnostics.reduce(
          (t, { location }) => Math.max(t, offsetStart(location).line),
          location.start.line
        );
      } else {
