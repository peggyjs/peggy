...
const peg = {
  ...
  generate(grammar, options) {
    ...
    if (!Array.isArray(grammar)) {
      console.error("generate(): grammar should be array, but it is", typeof grammar, grammar instanceof Uint8Array,"have to fix it.");
      grammar = [{
    ...
    const combined = asts.combine(
      grammar.map(({ source, encoding, arr_bytes, text }) => config.parser.parse(text, {
