  ...
  function generateToplevel() {
    ...
    parts.push(
      ...
      `
                ...
                // the offsets in this.location.start only work with the JavaScript UTF16 string 
                // and don't properly map to a UTF8 or binary file. we can figure out the UTF8 offset like this: 
                //  - take substring of UTF16 string until the error message
                //  - convert the substring from UTF16 to UTF8
                //  - length of the UTF8-converted substring is the offset we're looking for
                // (maybe there is more clever way to do this but I'm tired)
                let offset_error_in_utf8 = (new TextEncoder().encode(src.slice(0, this.location.start.offset))).length;
                ...
                // combine Uint8Array arrays to one large array
                let arr_bytes_with_error_context = new Uint8Array(arr_bytes_before_error_position.length + arr_bytes_at_error_position.length + arr_bytes_after_error_position.length);
                arr_bytes_with_error_context.set(arr_bytes_before_error_position);
                arr_bytes_with_error_context.set(arr_bytes_at_error_position, arr_bytes_before_error_position.length);
                arr_bytes_with_error_context.set(arr_bytes_after_error_position, arr_bytes_before_error_position.length + arr_bytes_at_error_position.length);
                ...
                // output to console
                str += ("\\n\\nError starts at offset " + offset_error_in_utf8
