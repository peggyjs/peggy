...
function generateJS(ast, options) {
  ...
  function toSourceNode(node) {
    return asts.toSourceNode(node, options, node.type);
  }
  ...
  function generateToplevel() {
    ...
    if (ast.topLevelInitializer) {
      parts.push(toSourceNode(ast.topLevelInitializer));
      parts.push("");
    }
    ...
    parts.push(
      ...
      "",
      indent2(generateTables().toString()),
      "",
      "  var peg$currPos = 0;",
      "  var peg$savedPos = 0;",
    ...
    if (ast.initializer) {
      parts.push(toSourceNode(ast.initializer));
      parts.push("");
    }
  ...
  ast.code = generateWrapper(generateToplevel().join("\n")).join("\n");
}
