...
const compiler = {
  ...
  compile(ast, passes, options) {
    ...
    switch (options.output) {
      case "parser":
        console.log("!!!! PARSER", ast.code.toString())
        // eslint-disable-next-line no-eval -- Required
        return eval(ast.code.toString());

      case "source":
        console.log("!!!! SOURCE", ast.code.toString())
        return ast.code.toString();

      case "source-and-map":
        console.log("!!!! SOURCE-AND-MAP", ast.code)
        return ast.code;

      case "source-with-inline-map": {
        console.log("!!!! source-with-inline-map");
        ...
        console.log("!!!! sourceMap.code", sourceMap.code)
