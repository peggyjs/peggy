...
$("#run").click(() => {
    ...
    Runner.run(benchmarks, runCount, options, {
        ...
        readBenchmark(file) {
            return $.ajax({
                type: "GET",
                url: `https://raw.githubusercontent.com/peggyjs/peggy/${BRANCH}/examples/${file}`,
                dataType: "text",
                async: false,
            }).responseText;
        },
        ...
        readTest(file) {
            return $.ajax({
                type: "GET",
                url: `/development/benchmark-data/${file}`,
                dataType: "text",
                async: false,
            }).responseText;
        },
        ...
        testFinish(benchmark, test, inputSize, parseTime) {
            appendResult(
                "individual",
                test.title,
                `https://github.com/peggyjs/peggy/blob/${BRANCH}/pkg/peggy-benchmarks/${benchmark.id}/${test.file}`,
                inputSize,
                parseTime
            );
        },
