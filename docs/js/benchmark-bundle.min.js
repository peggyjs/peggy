...
var BRANCH = location.hash.match(/branch=([^&]*)/i);
...
$("#run").click(function () {
  ...
  Runner.run(benchmarks, runCount, options, {
    readFile: function readFile(file) {
      return $.ajax({
        type: "GET",
        url: "https://raw.githubusercontent.com/peggyjs/peggy/".concat(BRANCH, "/benchmark/").concat(file),
        dataType: "text",
        async: false
      }).responseText;
    },
    testStart: function testStart() {// Nothing to do.
    },
    testFinish: function testFinish(benchmark, test, inputSize, parseTime) {
      appendResult("individual", test.title, "https://github.com/peggyjs/peggy/blob/".concat(BRANCH, "/benchmark/").concat(benchmark.id, "/").concat(test.file), inputSize, parseTime);
    },
    benchmarkStart: function benchmarkStart(benchmark) {
      resultsTable.append("\n                <tr class='heading'>\n                    <th colspan='4'>\n                        <a href='https://github.com/pegjs/pegjs/blob/".concat(BRANCH, "/examples/").concat(benchmark.id, ".pegjs'> ").concat(benchmark.title, " </a>\n                    </th>\n                </tr>\"\n            "));
    },
  ...
  run: function run(benchmarks, runCount, options, callbacks) {
    ...
    var Q = {
      ...
      run: function run() {
        if (this.functions.length > 0) {
          this.functions.shift()(); // We can't use |arguments.callee| here because |this| would get
    ...
    function benchmarkInitializer(benchmark) {
      return function () {
        ...
        state.parser = peg.generate(callbacks.readFile("../examples/" + benchmark.id + ".pegjs"), options);
...
var visitor = require("./visitor"); // AST utilities.
...
"use strict"; // Bytecode instruction opcodes.
...
var visitor = require("../visitor"); // Generates bytecode.
//
// Instructions
// ============
//
...
function generateBytecode(ast) {
  ...
  var generate = visitor.build({
    ...
    named: function named(node, context) {
      var nameIndex = addConst("peg$otherExpectation(\"" + js.stringEscape(node.name) + "\")"); // The code generated below is slightly suboptimal because |FAIL| pushes
    ...
    literal: function literal(node) {
      if (node.value.length > 0) {
        ...
        var expectedIndex = addConst("peg$literalExpectation(" + "\"" + js.stringEscape(node.value) + "\", " + node.ignoreCase + ")"); // For case-sensitive strings the value must match the beginning of the
...
var op = require("../opcodes"); // Generates parser JavaScript code.
...
function generateJS(ast, options) {
  ...
  function generateInterpreter() {
    ...
    parts.push([// The point of the outer loop and the |ips| & |ends| stacks is to avoid
  ...
  function generateWrapper(toplevelCode) {
    function generateGeneratedByComment() {
      return ["// Generated by Peggy 0.11.0.", "//", "// https://github.com/peggyjs/peggy"].join("\n");
    }
...
var visitor = require("../visitor"); // Removes proxy rules -- that is, rules that only delegate to other rule.
...
var visitor = require("../visitor"); // Checks that each label is defined only once within each scope.
...
var visitor = require("../visitor"); // Checks that each rule is defined only once.
...
var visitor = require("../visitor"); // Reports left recursion in the grammar, which prevents infinite recursion in
...
var visitor = require("../visitor"); // Reports expressions that don't consume any input inside |*| or |+| in the
...
var visitor = require("../visitor"); // Checks that all referenced rules exist.
...
"use strict"; // Simple AST node visitor builder.
...
"use strict"; // Thrown when the grammar contains an error.
...
module.exports={
  ...
  "scripts": {
    ...
    "browser:build": "browserify lib/peg.js -s peggy -o browser/peggy.js -t [ babelify --no-compact --presets [ @babel/preset-env ] ] && uglifyjs browser/peggy.js -o browser/peggy.min.js",
    "postbrowser:build": "tools/header browser/* && cp browser/peggy.min.js docs/vendor/peggy/peggy.min.js",
    "benchmark:build": "browserify benchmark/browser.stub.js -o docs/js/benchmark-bundle.min.js -t [ babelify --no-compact --presets [ @babel/preset-env ] ]",
    "test:build": "browserify test/browser.stub.js -o docs/js/test-bundle.min.js -t [ babelify --no-compact --presets [ @babel/preset-env ] ]",
